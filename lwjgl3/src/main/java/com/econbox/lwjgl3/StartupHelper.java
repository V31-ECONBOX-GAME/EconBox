...
public class StartupHelper {
	...
	/** Grants JNI and FFM access to the unnamed module, silencing the Java 24+ warning. */
	...
	public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
		...
		// The child JVM is started with -cp, not -jar, so a packaged JAR's
		// Enable-Native-Access manifest attribute does not carry over. Without re-adding
		// the flag, Java 24+ warns about restricted methods on every macOS launch.
		// (EconBox addition; not part of the upstream libGDX StartupHelper.)
		if (!jvmArgs.contains(ENABLE_NATIVE_ACCESS)) jvmArgs.add(ENABLE_NATIVE_ACCESS);
